...
const SortableProductItem = ({ post, id, onRemove }) => {
		...
		>
			{thumbUrl && (
				<img
					src={thumbUrl}
					alt=""
					style={{ width: 44, height: 44, objectFit: "contain", borderRadius: 4, flexShrink: 0, background: "#f5f5f5" }}
				/>
			)}
...
const Inspector = ({ attributes, setAttributes }) => {
	...
	const addProduct    = (id) => setAttributes({ selectedIds: [...selectedIds, id] });
	const removeProduct = (id) => setAttributes({ selectedIds: selectedIds.filter((i) => i !== id) });
	...
	return (
		<InspectorControls>
			...
			{/* ── Sản phẩm đã chọn ── */}
			<PanelBody title={__("Sản phẩm đã chọn", "flip-blocks")} initialOpen={true}>
				<p style={{ fontSize: 11, color: "#888", margin: "0 0 8px" }}>
					{__("Để trống = hiển thị tất cả sản phẩm.", "flip-blocks")}
				</p>
				{allPosts === null ? (
					...
					<p style={{ fontSize: 12, color: "#999", margin: 0 }}>
						{__("Chưa chọn sản phẩm nào — hiển thị tất cả.", "flip-blocks")}
					</p>
				) : (
					<>
			...
			{/* ── Thêm sản phẩm ── */}
			<PanelBody title={__("Thêm sản phẩm", "flip-blocks")} initialOpen={false}>
				{allPosts === null ? (
					<Spinner />
				) : (
					<>
						...
						{filteredUnselected.length === 0 ? (
							<p style={{ fontSize: 12, color: "#999", margin: 0 }}>
								{allPosts.length === selectedIds.length
									? __("Đã chọn tất cả sản phẩm.", "flip-blocks")
									...
									>
										{thumb ? (
											<img
												src={thumb}
												alt=""
												style={{ width: 40, height: 40, objectFit: "contain", borderRadius: 4, flexShrink: 0, background: "#f5f5f5" }}
											/>
										) : (
											<div style={{ width: 40, height: 40, background: "#f0f0f0", borderRadius: 4, flexShrink: 0 }} />
										)}
										<span style={{ flex: 1, fontSize: 12, color: "#1e1e1e" }}>
											{post.title?.rendered || "—"}
										</span>
										<button
											onClick={() => addProduct(post.id)}
